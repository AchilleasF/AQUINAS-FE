...
bnu::matrix<double> AQUINAS_BDTB_GN0G_matrix(double eta, double L, double r, double dphids, double c, double s, double psi, double Nphi, double Ntheta, double Nphitheta, const bnu::vector<double> elDOFs, const bnu::matrix<double> DT)
{
    double eta2 = eta * eta;  
    ...
    bnu::matrix<double> Ba = B0a + BLa;
    bnu::matrix<double> BaDT = prod(trans(Ba),DT);
    bnu::matrix<double> BaDTBa = prod(BaDT,Ba);
    bnu::matrix<double> GaN0 = prod(trans(Ga),N0);
    bnu::matrix<double> GaN0Ga = prod(GaN0,Ga);
    bnu::matrix<double> KT = BaDTBa + GaN0Ga;
    return KT;
}
